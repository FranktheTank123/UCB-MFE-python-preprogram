"""
Reference:
https://github.com/FranktheTank123/UCB-MFE-python-preprogram/tree/main/Homeworks/HW9/Frank_Xia
https://www.kaggle.com/humananalog/xgboost-lasso/script
...
from model_definition import regr 
...
def munge(df):
    ...
    qual_dict = {None: 0, np.nan: 0, "Po": 1, "Fa": 2, "TA": 3, "Gd": 4, "Ex": 5, }
    ...
    all_df["BsmtExposure"] = df["BsmtExposure"].map(
        {None: 0, np.nan: 0, "No": 1, "Mn": 2, "Av": 3, "Gd": 4}).astype(int)

    bsmt_fin_dict = {None: 0, np.nan: 0, "Unf": 1, "LwQ": 2, "Rec": 3, "BLQ": 4, "ALQ": 5, "GLQ": 6}
    all_df["BsmtFinType1"] = df["BsmtFinType1"].map(bsmt_fin_dict).astype(int)
    all_df["BsmtFinType2"] = df["BsmtFinType2"].map(bsmt_fin_dict).astype(int)

    all_df["Functional"] = df["Functional"].map(
        {None: 0, np.nan: 0, "Sal": 1, "Sev": 2, "Maj2": 3, "Maj1": 4, 
         "Mod": 5, "Min2": 6, "Min1": 7, "Typ": 8}).astype(int)

    all_df["GarageFinish"] = df["GarageFinish"].map(
        {None: 0, np.nan: 0, "Unf": 1, "RFn": 2, "Fin": 3}).astype(int)

    all_df["Fence"] = df["Fence"].map(
        {None: 0, np.nan: 0, "MnWw": 1, "GdWo": 2, "MnPrv": 3, "GdPrv": 4}).astype(int)
...
def munge_onehot(df):
    """
        Combine all the (one hot) features into one big DataFrame.
    """
...
def preping_data(data_location) -> tuple[pd.DataFrame, pd.Series]:
    ...
    # scale the data (need to be saved)
    ...
    return train_df_munged, label_df
...
def main(data_path, model_path):
    ...
    print("XGboost R square on training set: ", r2_score(y_test, y_pred))
    
    # save model
    regr.save_model(model_path)
    return 
