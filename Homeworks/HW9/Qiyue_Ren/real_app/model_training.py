...
def preping_data(data_location):
    ...
    X[f"ret_SOL_lag1"] = X.ret_SOL.shift(1)
    X[f"ret_SOL_lag3"] = X.ret_SOL.shift(3)
    X[f"ret_SOL_lag6"] = X.ret_SOL.shift(6)
...
def main(data_path, model_path):
    ...
    search = GridSearchCV(
        pipeline,
        {"pca__n_components": [1, 5, 10, 20, 30], "model__alpha": [0.1, 0.5, 1.0]},
        scoring=scorer,
        refit=True,
        cv=cv,
        n_jobs=-1,
